...
export function downloadGlyphsMetadata(urlTemplate: string, cb: (...args: any[]) => void) {
  ...
  if (urlObj.hostname === 'tiles.openfreemap.org') {
    // OpenFreeMap serves glyph PBFs but intentionally has no CORS-enabled
    // fonts.json metadata endpoint. The editor already supports an empty
    // metadata fallback, so avoid a guaranteed 403 and Firefox CORS warning.
    return cb([])
  }
