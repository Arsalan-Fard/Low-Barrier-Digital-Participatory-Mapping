...
type MapMaplibreGlInternalProps = {
  ...
  onMapReady?(map: Map | null): unknown
...
class MapMaplibreGlInternal extends React.Component<MapMaplibreGlInternalProps, MapMaplibreGlState> {
  ...
  componentDidUpdate(previousProps: MapMaplibreGlInternalProps) {
    const map = this.state.map;
    const mapStyleChanged = previousProps.mapStyle !== this.props.mapStyle;
    const inspectModeChanged = previousProps.inspectModeEnabled !== this.props.inspectModeEnabled;
    const highlightedLayerChanged = previousProps.highlightedLayer !== this.props.highlightedLayer;

    if (map && mapStyleChanged) {
    ...
    if (map && previousProps.options !== this.props.options) {
    ...
    if (
      this.state.inspect
      && this.props.inspectModeEnabled
      && (mapStyleChanged || inspectModeChanged || highlightedLayerChanged)
    ) {
      const styleWithTokens = this.props.replaceAccessTokens(this.props.mapStyle);
