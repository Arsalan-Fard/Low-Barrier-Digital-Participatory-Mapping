...
function metadataLabel(layer: LayerSpecification, key: string) {
...
class LayerListContainerInternal extends React.Component<LayerListContainerInternalProps, LayerListContainerState> {
  ...
  render() {
    ...
    layersByGroup.forEach(layers => {
      const groupPrefix = layerPrefix(layers[0].id)
      const groupTitle = (
        metadataLabel(layers[0], "paper:poi-category-label")
        || groupPrefix
      );
      if(layers.length > 1) {
      ...
      layers.forEach((layer, idxInGroup) => {
        ...
        const listItem = <LayerListItem
          ...
          id={layer.key}
          layerId={layer.id}
          layerLabel={metadataLabel(layer, "paper:poi-rank-label")}
          layerIndex={idx}
...
const LayerList: React.FC<LayerListProps> = (props) => {
  const sensors = useSensors(useSensor(PointerSensor, {
    // Keep ordinary layer selection clicks from becoming accidental drags.
